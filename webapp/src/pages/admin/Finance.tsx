...
export default function Finance() {
  ...
  const payReportUrl = zoneId
    ? `/api/pay-report?zoneId=${encodeURIComponent(zoneId)}`
    : "/api/pay-report";

  const { data: report, isLoading, isError, dataUpdatedAt } = useQuery<PayReport>({
    queryKey: ["pay-report", zoneId ?? "all"],
    queryFn: () => api.get<PayReport>(payReportUrl),
    enabled: !!profile && canAccessPayroll(profile.role),
  });
  ...
  const { mutate: approve, isPending: approving, variables: approvingId } = useMutation({
    ...
    onSuccess: (result) => {
      ...
      toast.success(
        `${result.approved_count} assignment${result.approved_count !== 1 ? "s" : ""} approved for payout.`
      );
    },
    onError: (e: Error) => {
      toast.error(e.message.includes("401") ? "Sign-in required." : e.message);
    },
  });
  ...
  const zoneLabel =
    zoneId != null
      ? zones.find((z) => z.id === zoneId)?.name ?? report?.zone_name ?? "Zone"
      : "All zones";
      ...
      <div className="space-y-6 max-w-6xl">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Confirmed assignments ready for payout review. Filter by zone or view all zones in the
              province.
              {report?.season ? (
        ...
        <div className="flex flex-wrap items-end gap-3 rounded-lg border border-border bg-muted/30 px-4 py-3">
          ...
          <p className="text-xs text-muted-foreground pb-1">
            Showing payouts for <span className="font-medium text-foreground">{zoneLabel}</span>
          </p>
        </div>
          ...
          <div className="flex flex-col items-center gap-3 py-20 text-muted-foreground">
            ...
            <div className="text-center">
              ...
              <p className="text-xs mt-0.5 max-w-sm">
                {zoneId
                  ? `No confirmed games in ${zoneLabel} for this season yet.`
                  : "Assignments appear here once their status is Confirmed."}
              </p>
            </div>
          </div>
        ) : (
