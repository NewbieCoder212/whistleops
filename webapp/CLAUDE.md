...
<backend>
  Hono API at ../backend/. Supabase Postgres + Auth.

  Use the typed resource clients in src/lib/resources.ts:
  ```typescript
  import { gamesApi, profilesApi } from "@/lib/resources";
  const games = await gamesApi.list();
  ```

  Or the raw fetch wrapper for ad-hoc endpoints:
  ```typescript
  import { api } from "@/lib/api";
  const data = await api.get<MyType>("/api/something");
  ```

  src/lib/api.ts automatically attaches the Supabase access token as a
  Bearer header — no manual auth wiring needed.

  Auth uses Supabase directly from the browser:
  ```typescript
  ...
  <shared_types>
    ...
    Import via the `@shared` alias:
    ```typescript
    import type { Game, Profile, RoleEnum } from "@shared/types";
    ```
  </shared_types>

  <env>
    Needs VITE_SUPABASE_URL, VITE_SUPABASE_ANON_KEY (set via Vibecode ENV tab).
    VITE_BACKEND_URL only needed in dev when backend is on a different port.
  </env>
</backend>
